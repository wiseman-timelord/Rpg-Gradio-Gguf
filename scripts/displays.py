...
/* Status bar - allow horizontal scrolling for long workflow text */
            ...
            with gr.Tab("Conversation"):
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        with gr.Column(visible=True) as interaction_panel:
                            ...
                            # Row with Send and Cancel buttons - BOTH ALWAYS VISIBLE, STATIC
                            with gr.Row():
                                send_btn = gr.Button(
                                    "Send Message",
                                    variant="primary",
                                    scale=3,      # larger (3 parts)
                                    interactive=True,
                                )
                                cancel_btn = gr.Button(
                                    "Cancel Response",
                                    variant="stop",
                                    scale=1,      # smaller (1 part)
                                    interactive=True,  # Always clickable - cancel flag handles it
                                )
                ...
                with gr.Row():
                    conv_status = gr.Textbox(
                        ...
                        label="Status",
                        interactive=False,
                        max_lines=2,  # Changed from 1 to allow wrapping if needed
                        scale=20,
                        elem_id="conv_status",
                    )
                ...
                left_mode.change(
                    ...
                    outputs=[
                        ...
                        happenings_panel,
                        personalize_panel,
                        # NO button outputs - they remain static
                    ],
                )
                ...
                cancel_btn.click(
                    fn=cancel_response,
                    inputs=[],
                    outputs=[conv_status],  # Only updates status
                )
