...
- Text model - [Qwen3-4B-abliterated-GGUF](https://huggingface.co/sothmik/Wai-NSFW-Illustrious-v140-Q8-GGUF) (as you chose). Quant level: aim for a Q4‑class quant (e.g. Q4_K_M or similar) so it sits around ~2–3 GB. That keeps enough headroom for the image model and some system VRAM. This gives you your uncensored RPG brain.
...
1. The installer needs to install a lightweight browser into the venv, in order for the program to have its own application style GUI. [X] on top right must do same as "Exit Program" button using proper shutdown method.
2. When I run the program without debug option, it is supposed to run the gradio server and python scripts in the background (but without terminal/console open), and load the GUI. Currently its not doing anything, possibly silently exiting. 
