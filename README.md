...
- Text model - Model: Qwen3-4B-abliterated-GGUF (as you chose). Quant level: aim for a Q4‑class quant (e.g. Q4_K_M or similar) so it sits around ~2–3 GB. That keeps enough headroom for the image model and some system VRAM. This gives you your uncensored RPG brain.
- Image model - (GGUF, NSFW, ≤4 GB). Model: waiNSFWIllustrious_v140-Q8_0.gguf. WAI‑NSFW‑Illustrious‑SDXL v14.0, ~2.7 GB GGUF). ~3B params, SDXL architecture, anime‑leaning, explicitly NSFW‑capable. Integrated VAE, so you don’t need a separate VAE file, which is nice for a compact setup. Tuned on Danbooru‑style data with emphasis on pose stability and composition; good for character‑heavy RPG art (portraits, full‑body shots, etc.).
